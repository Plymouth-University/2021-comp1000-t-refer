...
namespace Tests
{
    public class Tests
    {
        ...
        public void InitialCheckTestInit()
        {
            ...
            Assert.True(crawler.GetGameState() == Game.GameState.INIT,"The game should not be running as we have not loaded the map yet");
        ...
        public void InitialCheckTestInput()
        {
            ...
            Assert.True(crawler.GetGameState() == Game.GameState.INIT, "The game should not be running as we have not loaded the map correctly");
        ...
        public void InitialCheckTestGameInit()
        {
            ...
            Assert.True(crawler.GetGameState() == Game.GameState.START, "The game should not be running as we have a map and the user command play was used.");
            char[][] curr = crawler.GetActiveMap();
        ...
        public void InitialCheckTestGameStart()
        {
            ...
            Assert.True(crawler.GetOriginalMap().Length == 10, "Map loading is not working unsing the load command ");
            Assert.True(crawler.GetOriginalMap()[0].Length == 31, "Map loading is not working unsing the load command ");
            Assert.True(crawler.GetGameState() == Game.GameState.START, "The game should not be running as we have a map and the user command play was not used.");
            crawler.ProcessUserInput("begin");
            Assert.True(crawler.GetGameState() == Game.GameState.RUN, "The game should now be running as we have a map and the user command play was used.");
        }
        ...
        public void TestUpdatedPlayerPosition()
        {
            ...
            char player = crawler.GetActiveMap()[pos[0]][pos[1]];
        ...
        public void TestPlayerActions()
        {
            ...
            crawler.ProcessUserInput("begin");
            Assert.True(crawler.GetPlayerAction() == Game.PlayerActions.NOTHING, "No player action should be received yet.");
            Assert.True(crawler.GetGameState() == Game.GameState.RUN, "The game should be running as we have a map and the user command play was used.");
            crawler.ProcessUserInput("A");
            Assert.True(crawler.GetPlayerAction() == Game.PlayerActions.EAST, "Game is Active and player triggered moving using <A> but not the correct action was triggered");
            crawler.ProcessUserInput("W");
            Assert.True(crawler.GetPlayerAction() == Game.PlayerActions.NORTH, "Game is Active and player triggered moving using <W> but not the correct action was triggered");
            crawler.ProcessUserInput("D");
            Assert.True(crawler.GetPlayerAction() == Game.PlayerActions.WEST, "Game is Active and player triggered moving using <D> but not the correct action was triggered");
            crawler.ProcessUserInput("P");
            Assert.True(crawler.GetPlayerAction() == Game.PlayerActions.PICKUP, "Game is Active and player triggered pickup using <P> but not the correct action was triggered");
            crawler.ProcessUserInput("S");
            Assert.True(crawler.GetPlayerAction() == Game.PlayerActions.SOUTH, "Game is Active and player triggered moving using <S> but not the correct action was triggered");
            crawler.ProcessUserInput("begin");
            Assert.True(crawler.GetPlayerAction() == Game.PlayerActions.NOTHING, "Game is Active and player typed in play again which should do nothing.");
        }
        ...
        public void TestGameWithWrongOrder()
        {
            ...
            //crawler.ProcessUserInput("begin");
            int[] pos = crawler.GetPlayerPosition();
            Assert.False(crawler.GetGameState() == Game.GameState.RUN, "The game should not be running as we have a map and the user command play was not used.");
            crawler.ProcessUserInput("W");
        ...
        [Fact]
        public void TestGameWithWrongInput()
        {
            Setup();
            crawler.ProcessUserInput("load Simple.map");
            char[][] orig = crawler.GetOriginalMap();
        ...
        public void TestMovementCounter()
        {
            ...
            Assert.True(crawler.GetPlayerAction() == Game.PlayerActions.NOTHING, "No player action should be received yet.");
            crawler.ProcessUserInput("W");
            Assert.True(crawler.GetPlayerAction() == Game.PlayerActions.NOTHING, "Dven though player used a movement command the game is not active. " +
                "thus, no playeraction should be triggered.");
            crawler.ProcessUserInput("begin");
        ...
        public void TestGamePlayFinish()
        {
            ...
            //reaching the DDit
            Assert.True(crawler.GetGameState() == Game.GameState.STOP, "The game should finished as the player reached the DDit.");
